...
const Courses = () => {
  ...
  const fetchEnrolledCourses = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setEnrollments(data); // Lưu toàn bộ enrollment data
      }
    } catch (err) {
  ...
  const getFilteredCourses = () => {
    if (filter === "in-progress") {
      return courses.filter((course) => isEnrolled(course._id) && !isCompleted(course._id));
    }
    if (filter === "completed") {
      ...
      <section className="courses">
        <div className="container">
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: "25px",
                  background: filter === "all" ? "linear-gradient(135deg, #3b82f6, #1e40af)" : "white",
                  color: filter === "all" ? "#fff" : "#1e40af",
                  border: "2px solid var(--primary)",
                  fontWeight: "600",
                  transition: "all 0.3s",
                  boxShadow: filter === "all" ? "0 4px 12px rgba(59, 130, 246, 0.18)" : "none",
                  transform: filter === "all" ? "translateY(-2px)" : "none",
                  textShadow: filter === "all" ? "0 1px 2px rgba(30,64,175,0.18)" : "none"
                }}
              >
              ...
              <button
                ...
                className={`btn ${filter === "in-progress" ? "btn-success" : "btn-outline"}`}
                style={{
                  padding: "0.75rem 1.5rem",
                  borderRadius: "25px",
                  background: filter === "in-progress" ? "linear-gradient(135deg, #10b981, #059669)" : "white",
                  color: filter === "in-progress" ? "#fff" : "#059669",
                  border: "2px solid #10b981",
                  fontWeight: "600",
                  transition: "all 0.3s",
                  boxShadow: filter === "in-progress" ? "0 4px 12px rgba(16, 185, 129, 0.18)" : "none",
                  transform: filter === "in-progress" ? "translateY(-2px)" : "none",
                  textShadow: filter === "in-progress" ? "0 1px 2px rgba(5,150,105,0.18)" : "none"
                }}
              >
                <i className="fas fa-play-circle"></i> Đang học ({enrollments.filter(e => !e.completed).length})
              </button>
              <button
                onClick={() => setFilter("completed")}
                className={`btn ${filter === "completed" ? "btn-warning" : "btn-outline"}`}
                style={{
                  padding: "0.75rem 1.5rem",
                  borderRadius: "25px",
                  background: filter === "completed" ? "linear-gradient(135deg, #f59e0b, #d97706)" : "white",
                  color: filter === "completed" ? "#fff" : "#d97706",
                  border: "2px solid #f59e0b",
                  fontWeight: "600",
                  transition: "all 0.3s",
                  boxShadow: filter === "completed" ? "0 4px 12px rgba(245, 158, 11, 0.18)" : "none",
                  transform: filter === "completed" ? "translateY(-2px)" : "none",
                  textShadow: filter === "completed" ? "0 1px 2px rgba(217,119,6,0.18)" : "none"
                }}
              >
                <i className="fas fa-check-double"></i> Đã hoàn thành ({enrollments.filter(e => e.completed).length})
              </button>
            </div>
          )}
          ...
          {!loading && !error && filteredCourses.length === 0 && courses.length > 0 && (
            <div style={{ textAlign: 'center', padding: '4rem' }}>
              <i className="fas fa-filter" style={{ fontSize: '4rem', color: 'var(--muted)', marginBottom: '1rem' }}></i>
              <h3 style={{ color: 'var(--muted)' }}>Không tìm thấy khóa học</h3>
              <p style={{ color: 'var(--muted)' }}>
                {filter === "in-progress" && "Bạn chưa có khóa học nào đang học."}
                {filter === "completed" && "Bạn chưa hoàn thành khóa học nào."}
              </p>
            </div>
          )}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                      ...
                      {enrollment && (
                        <span style={{
                          ...
                          borderRadius: "12px",
                          background: enrollment.completed ? "#fef3c7" : "#dbeafe",
                          color: enrollment.completed ? "#92400e" : "#1e40af",
                          fontWeight: "600"
                        }}>
                          {enrollment.completed ? (
                            <><i className="fas fa-check-double"></i> Hoàn thành</>
                          ) : (
                            <><i className="fas fa-play-circle"></i> {enrollment.progress || 0}%</>
                          )}
                        </span>
                      )}
                    </div>
